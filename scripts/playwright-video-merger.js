...
if (!fs.existsSync(FREEZE_FRAMES_DIR))
  fs.mkdirSync(FREEZE_FRAMES_DIR, { recursive: true });
...
console.log(
  "Processing Playwright test videos with a 1-second delay and wrapped subtitles..."
);
...
videoFiles.forEach((videoFile, index) => {
  ...
  const freezeFrameFile = path.join(
    FREEZE_FRAMES_DIR,
    `${testName}_freeze.mp4`
  );
