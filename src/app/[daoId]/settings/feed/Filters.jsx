...
import { getProposalApprovers, getProposalProposers } from "@/api/indexer";
import FiltersDropdown from "@/components/dropdowns/FiltersDropdown";
import { logger } from "@/helpers/logger";

const SettingsFeedFilters = ({
  activeFilters,
  setActiveFilters,
  ...
  useEffect(() => {
    if (treasuryDaoID) {
      getProposalApprovers(treasuryDaoID)
        ...
        .catch((err) => {
          logger.error("Error fetching approvers:", err);
        });
      getProposalProposers(treasuryDaoID)
        .then(setProposerOptions)
        .catch((err) => {
          logger.error("Error fetching proposers:", err);
        });
    }
  }, [treasuryDaoID]);
...
export default SettingsFeedFilters;
