...
const CONFIG = {
  ...
  BULK_PAYMENT_CONTRACT_ID:
    process.env.BULK_PAYMENT_CONTRACT_ID || "bulk-payment.sandbox",
...
try {
  ...
  while (!allPaid && attempts < maxAttempts) {
    ...
    const progress = ((list.paid_payments / list.total_payments) * 100).toFixed(
      1
    );
    console.log(
      `📊 Progress: ${list.paid_payments}/${list.total_payments} (${progress}%)`
    );
  ...
  for (const balance of balances) {
    if (balance.received) {
      successCount++;
      console.log(
        `✅ ${balance.recipient.substring(0, 16)}...: ${formatNEAR(balance.actual)} NEAR`
      );
    } else {
      console.log(
        `❌ ${balance.recipient.substring(0, 16)}...: ${formatNEAR(balance.actual)} NEAR (expected ${formatNEAR(balance.expected)})`
      );
    }
  }
  ...
  console.log(
    `Sample Recipients Verified: ${successCount}/${sampleRecipients.length}`
  );

  const finalStatus = await viewPaymentList(account, listId);
  const paidCount = finalStatus.payments.filter(
    (p) => p.status === "Paid"
  ).length;
  const pendingCount = finalStatus.payments.filter(
    (p) => p.status === "Pending"
  ).length;
  const failedCount = finalStatus.payments.filter(
    (p) => p.status && p.status.Failed
  ).length;
  ...
  assert.equal(
    paidCount,
    CONFIG.NUM_RECIPIENTS,
    `All ${CONFIG.NUM_RECIPIENTS} payments must be Paid`
  );
  ...
  assert.equal(
    ...
    "All sample recipients must have received their tokens"
  );
