...
test.describe("Payment Request Detail Page", () => {
  test("displays Intents payment request for ETH with all details", async ({
    page,
  }) => {
    ...
    await expect(page.getByText("Transaction Links").first()).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(page.getByText("View transfer on etherscan.io")).toBeVisible({
      timeout: 10000,
    });
    console.log("✓ etherscan.io link is visible");

    console.log(
      "✓ Intents payment request for ETH displays all details correctly"
    );
  });

  test("displays Intents payment request for wNEAR (NEAR-to-NEAR)", async ({
    page,
  }) => {
    // Navigate to an Intents payment for wNEAR (NEAR-to-NEAR)
    // Proposal ID 4: 0.2 wNEAR to petersalomonsen.near
    await page.goto(
      `http://localhost:3000/${DAO_ID}/payments?tab=history&id=4`,
      { waitUntil: "networkidle" }
    );
    ...
    const networkValue = networkLabel
      .locator("..")
      .locator("span.text-capitalize");
    ...
    const externalChainLink = page.getByText(
      /View.*on (etherscan|polygonscan|bscscan)/i
    );
    await expect(externalChainLink).not.toBeVisible();
    console.log("✓ No external chain links (as expected for NEAR-to-NEAR)");

    console.log(
      "✓ Intents payment request for wNEAR displays all details correctly"
    );
  });
  ...
  test("displays regular (non-intents) payment request", async ({ page }) => {
    ...
    await page.goto(
      `http://localhost:3000/${DAO_ID}/payments?tab=history&id=8`,
      { waitUntil: "networkidle" }
    );
    ...
    await expect(externalChainLink).not.toBeVisible();
    console.log(
      "✓ No external chain links (as expected for regular NEAR payments)"
    );

    console.log("✓ Regular payment request displays all details correctly");
  });

  test("displays failed payment request with transaction links", async ({
    page,
  }) => {
    // Navigate to a failed payment request
    // Proposal ID 0: Failed payment
    await page.goto(
      `http://localhost:3000/${DAO_ID}/payments?tab=history&id=0`,
      { waitUntil: "networkidle" }
    );
    ...
    await expect(
      page.getByText("Transaction Links", { exact: true })
    ).toBeVisible({ timeout: 10000 });
    console.log("✓ Transaction Links section is visible");
    ...
    console.log(
      "✓ Failed payment request displays transaction links correctly"
    );
  });
});
