...
test.describe("Base Payment Request Detail", () => {
  test.beforeAll(async () => {
    ...
    creatorAccountId = await sandbox.createAccount("testcreator.near", await parseNEAR("10"));
  ...
  test("creates Base USDC payment request and logs proposal structure", async ({ page }) => {
    ...
    await expect(offcanvas.getByText("Create Payment Request")).toBeVisible({ timeout: 10000 });
    console.log("✓ Payment request form opened");

    // Select NEAR Intents wallet
    const walletSelector = offcanvas.getByRole("button", { name: "Select Wallet" });
    ...
    const tokenDropdown = offcanvas.locator('button:has-text("Select Token"), [data-testid="token-dropdown"]').first();
    ...
    // Fill in title
    await page.getByRole('textbox', { name: 'Title' }).fill("Test Base Payment");
    console.log("✓ Entered title");

    // Fill in summary
    await page.getByRole('textbox', { name: 'Summary' }).fill("Testing Base USDC payment");
    ...
    const recipientInput = page.getByRole('textbox', { name: /Enter.*Address/ });
    ...
    const decodedArgs = JSON.parse(Buffer.from(argsBase64, 'base64').toString());

    console.log("\n=== DECODED ARGS ===");
    console.log(JSON.stringify(decodedArgs, null, 2));

    // Fetch supported tokens to see how the network info maps
    const supportedTokensResponse = await fetch("https://bridge.chaindefuser.com/rpc", {
      method: "POST",
    ...
    expect(decodedArgs.token).toContain("base-0x833589fcd6edb6e08f4c7c32d4f71b54bda02913");
    expect(decodedArgs.memo).toContain("WITHDRAW_TO:0x742d35Cc6634C0532925a3b844Bc454e4438f44e");
    ...
    await expect(page.getByText("0x742d35Cc6634C0532925a3b844Bc454e4438f44e")).toBeVisible({ timeout: 10000 });
