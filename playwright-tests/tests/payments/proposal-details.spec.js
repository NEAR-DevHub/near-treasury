...
test.describe("Payment Request Detail Page - Full Page View", () => {
  test("displays regular payment request with all details (full page)", async ({
    page,
  }) => {
  ...
  test("displays failed payment request correctly (full page)", async ({
    page,
  }) => {
  ...
  test("navigates back from full page detail to payments list", async ({
    page,
  }) => {
    await page.goto(
      `http://localhost:3000/${DAO_ID}/payments?tab=history&id=8`,
...
test.describe("Payment Request Detail Page - Compact View (Overlay)", () => {
  test("opens compact view when clicking on history proposal row", async ({
    page,
  }) => {
    ...
    const rowVisible = await proposalRow
    ...
    await expect(
      page.getByText(/Payment Request (Approved|Failed|Rejected)/)
